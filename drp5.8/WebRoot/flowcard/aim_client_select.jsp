...
<form name="clientForm" >
  ...
  <table width="95%" border="1" cellspacing="0" cellpadding="0" align="center" class="table1">
    ...
    %>
    <tr > 
      <td class="rd8" ><input type="radio" name="selectFlag" value="radiobutton" ondblclick="doubleclick('<%=aimClient.getId() %>', '<%=aimClient.getTemiOrClientId()%>', '<%=aimClient.getName()%>')"></td>
      <td  class="rd8" ><%=aimClient.getTemiOrClientId() %></td>
      <td  class="rd8" ><%=aimClient.getName() %></td>
      <td  class="rd8" ><%=aimClient.getLevelName() %></td>
    </tr>
    <%
    	}
    %>
