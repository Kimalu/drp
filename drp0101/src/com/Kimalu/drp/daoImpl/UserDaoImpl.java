...
public class UserDaoImpl extends  AbstractBaseDao<User> {
	...
	public List<User> query(Integer pageNO ,Integer pageSize) {
		...
		if(pageNO==null || pageSize==null){
			...
			sql="select t.* from (select rownum rn,  u.user_id,u.user_name,u.password,u.contact_tel,u.email,u.create_date from(select rownum rn, u.* from t_user u) u where rn>1) t where rn>=? and rn<=?";
			try {
				ps=conn.prepareStatement(sql);
