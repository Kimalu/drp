...
<script type="text/javascript">
	...
	function bottomPage() {
		window.self.location = "item_select.jsp?index=<%=index%>&pageNo=<%=pageModel.getButtomPageNo()%>&itemNoOrName=<%=queryStr%>";
	}
...
<form name="itemForm" >
  ...
  <table width="95%" border="1" cellspacing="0" cellpadding="0" align="center" class="table1">
    ...
    %>	
    <tr> 
      <td class="rd8" ><input type="radio" name="selectFlag" value="radiobutton" ondblClick="doubleclick('<%=item.getItemNo()%>', '<%=item.getItemName()%>', '<%=item.getSpec()%>', '<%=item.getPattern()%>', '<%=item.getItemUnit().getName()%>')"></td>
  ...
  <table width="95%" height="30" border="0" align="center" cellpadding="0" cellspacing="0" class="rd1">
    <tr> 
      <td  nowrap class="rd19" height="2" width="36%"> <div align="left"><font color="#FFFFFF">&nbsp;共&nbsp<%=pageModel.getTopPageNo() %>&nbsp页</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="#FFFFFF">当前第</font>&nbsp<font color="#FF0000"><%=pageModel.getPageNo() %></font>&nbsp<font color="#FFFFFF">页</font> 
        </div></td>
