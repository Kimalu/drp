...
<script type="text/javascript">
	...
	function topPage() {
		if(currentPageNo!=1){
			var url="<%=request.getContextPath()%>/basedata/FiscalYearPeriodController?command=query&pageNo=1&random="+Math.random();
			queryAjax(url);
		}
	}
	...
	function previousPage() {
		...
		if(currentPageNo!=1){
			...
			var url="<%=request.getContextPath()%>/basedata/FiscalYearPeriodController?command=query&pageNo="+pn+"&random="+Math.random();
			queryAjax(url);
		}
	}
	...
	function bottomPage() {
		if(currentPageNo!=totalPageNum){
			var url="<%=request.getContextPath()%>/basedata/FiscalYearPeriodController?command=query&pageNo="+ totalPageNum + "&random=" + Math.random();
			queryAjax(url);
		}
	}
</script>
...
<table width="95%" height="30" border="0" align="center" cellpadding="0" cellspacing="0" class="rd1">
	<tr>
		...
		<div align="left"><font color="#FFFFFF">&nbsp;共<span id="totalPageNum"></span>页</font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font color="#FFFFFF">当前第</font>&nbsp <font color="#FF0000"><span
			id="currentPageNo"></span></font>&nbsp <font color="#FFFFFF">页</font></div>
		</td>
		...
		<div align="right"><input name="btnTopPage" class="button1" type="button" id="btnTopPage" value="|&lt;&lt; " title="首页" onclick="topPage()"> <input name="btnPreviousPage"
			...
			name="btnAdd" type="button" class="button1" id="btnAdd" value="添加" onClick="add()"> <input name="btnModify" class="button1" type="button" id="btnModify" value="修改"
			onClick="self.location='fiscal_year_period_modify.html'"></div>
		</td>
	</tr>
</table>
<p>&nbsp;</p>
